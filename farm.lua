local VirtualUser = game:GetService('VirtualUser') game:GetService('Players').LocalPlayer.Idled:Connect(function() VirtualUser:CaptureController() VirtualUser:ClickButton2(Vector2.new()) end) game:GetService("StarterGui"):SetCore("SendNotification", { Title = "Made by Exotic Scripts!", Text = "Report bugs in the comments on our channel! 🤫", Button1 = "Ok", Duration = 30 })
-- SERVICES
...
local function createToggle(text, y, default, callback)
	...
	hit.MouseButton1Click:Connect(function()
		...
		TweenService:Create(toggle, TweenInfo.new(0.2), {BackgroundColor3 = state and Color3.fromRGB(60,200,120) or Color3.fromRGB(200,70,70)}):Play()
...
local initialX = (speed - minSpeed) / (maxSpeed - minSpeed)
...
knobS.Position = UDim2.new(initialX, -8, 0.5, -8)
...
valueLabel.Position = UDim2.new(0,0,0,240)
...
warning.Text = "I do not recommend setting it higher than 21. Anti-cheat may kick you out of the game."
warning.Font = Enum.Font.Gotham
warning.TextSize = 11
warning.TextColor3 = Color3.fromRGB(255, 100, 100)
warning.TextTransparency = 0.4 -- Półprzezroczysty
warning.BackgroundTransparency = 1
warning.TextWrapped = true
warning.TextYAlignment = Enum.TextYAlignment.Bottom
...
UserInputService.InputChanged:Connect(function(i)
	if sliding and i.UserInputType == Enum.UserInputType.MouseMovement then updateSlider() end
end)
...
task.spawn(function()
	while _G.Running do
		if _G.Active then
			if _G.AutoReset and coinCount >= maxCoins then
				_G.Active = false
			...
			if root then
				...
				for _,v in pairs(workspace:GetDescendants()) do
					if (v.Name=="Coin" or v.Name=="Coin_Server") and v:IsA("BasePart") and not ignored[v] then
